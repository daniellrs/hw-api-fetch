...
export default class HwResponse {
  ...
  constructor(response) {
    ...
    isErroValidacao = () => {
      return this.response.status === 'ERRO_VALIDACAO' || this.response.status === 'VALIDATION_ERROR'
    };

    // Retorna se houve algum erro no request, como url invalida ou algo do tipo
    isErroRequest = () => {
      return this.response.status === 'REQUEST_INVALIDO' || this.response.status === 'REQUEST_ERROR'
    };
    ...
    isErroInterno = () => {
      return this.response.status === 'ERRO_INTERNO' || this.response.status === 'INTERNAL_ERROR'
    };
    ...
    getErroRecusado = () => {
      ...
      for (var i = 0; i < length; i++) {
        if (this.response.erros[i].tipo === 'RECUSADO') { return this.response.erros[i] }
      }
      return false
    };
    ...
    hasErroAuth = () => {
      ...
      var length = this.response.erros.length
      for (var i = 0; i < length; i++) {
        if (this.response.erros[i].tipo === 'AUTH') { return true }
      }
      return false
    };
    ...
    getErroAuth = () => {
      ...
      var length = this.response.erros.length
      for (var i = 0; i < length; i++) {
        if (this.response.erros[i].tipo === 'AUTH') { return this.response.erros[i] }
      }
      return false
    };
    ...
    hasErroRequest = () => {
      ...
      var length = this.response.erros.length
      for (var i = 0; i < length; i++) {
        if (this.response.erros[i].tipo === 'REQUEST') { return true }
      }
      return false
    };
    ...
    getErroRequest = () => {
      ...
      var length = this.response.erros.length
      for (var i = 0; i < length; i++) {
        if (this.response.erros[i].tipo === 'REQUEST') { return this.response.erros[i] }
      }
      return false
    };
    ...
    hasErroCamposInvalidos = () => {
      ...
      var length = this.response.erros.length
      for (var i = 0; i < length; i++) {
        if (this.response.erros[i].tipo === 'CAMPOS_INVALIDOS') { return true }
      }
      return false
    };
    ...
    getCamposInvalidos = () => {
      if (this.camposInvalidos !== null) { return this.camposInvalidos }
    ...
    */
    getErroCampo = (nomeCampo) => {
      if (!this.getCamposInvalidos().includes(nomeCampo)) { return null }
      var obj = this.getObjCamposInvalidos()
    ...
    getConteudo = () => {
      const conteudo = this.response.conteudo || this.response.content
      if (conteudo === null || conteudo === '') { return {} }
      return conteudo
    };

    // Retorna erros de request (nodejs)
    getRequestErrors = () => {
      return this.response.errors
    }

    // Retorna erros de validacao (nodejs)
    getValidationErrors = () => {
      return this.response.fieldErrors
    }

    // Retorna o conteudo cru do request
    getJson = () => this.response || {}
    ...
    getObjCamposInvalidos = () => {
      if (!this.hasErroCamposInvalidos()) { return null }
      var length = this.response.erros.length
      for (var i = 0; i < length; i++) {
        if (this.response.erros[i].tipo === 'CAMPOS_INVALIDOS') { return this.response.erros[i] }
      }
    };
}
